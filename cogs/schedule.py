...
TIME_ERROR_EMBED = Embed(title="Time ran out. Try again. :anguished: ", color=Colour.red())
...
class Schedule(commands.Cog):
    ...
    def getEventEmbed(self, event):
        ...
        embed.add_field(name="Time", value=f"Start: {event['time']}\n Duration: {event['duration']}", inline=False)
    ...
    async def on_raw_reaction_add(self, payload):
        ...
        if any(event["messageId"] == payload.message_id for event in events) and self.bot.ready and not payload.member.bot:
            ...
            removeReaction = True
            event = [event for event in events if event["messageId"] == patload.message_id][0]
            ...
            if payload.emoji.name == "🟩":
                ...
                if payload.member.id == event["authorId"] or any(role.id == UNIT_STAFF for role in payload.member.roles):
                    await self.editEvent(event)
            elif payload.emoji.name == "🗑":
                if payload.member.id == event["authorId"] or any(role.id == UNIT_STAFF for role in payload.member.roles):
                    await self.deleteEvent(ctx, message)
                    events.remove(event)
    ...
    async def editEvent(self, event):
        pass
    
    async def deleteEvent(self, ctx, message):
        msg = await ctx.author.send("Are you sure you want to delete this event?")
        ...
        try:
            ...
            await ctx.author.send(embed=TIME_ERROR_EMBED)
            return
    ...
    async def schedule(self, ctx: SlashContext):
        if not DEBUG:
            await ctx.author.send("Schedule comming soon")
            return
        await ctx.send("Scheduling event, Standby for boperation")
        ...
        try:
            ...
            await dmChannel.send(embed=TIME_ERROR_EMBED)
            return
        ...
        if ctx.author.id in memberTimeZones:
            timeZone = pytz.timezone(memberTimeZones[ctx.author.id])
        else:
            ...
            try:
                ...
                if saveTimeZonepreference:
                    memberTimeZone[ctx.member.id] = timeZone.zone
        ...
        try:
            ...
            return
        try:
            eventTime = datetimeParse(eventTime).strftime(EVENT_TIME_FORMAT)
            formatOK = True
        except ValueError:
            formatOK = False
        while not formatOk:
    ...
    async def changeTimeZone(self, ctx: SlashContext):
        ...
        embed.add_field(name="Time Zone", value="\n".join(f"**{idx}** {tz}" for idx, tz in enumerate(TIME_ZONES, 1)))
        embed.set_footer("Enter `cancel` to keep your current preference")
        msg = await ctx.author.send(embed=embed)
        dmChannel = msg.channel
        try:
            response = await self.bot.wait_for("message", timeout=600, check=lambda msg, ctx=ctx, dmChannel=dmChannel: msg.channel == dmChannel and msg.author == ctx.author)
            ...
            if timeZone.isdigit() and int(timeZone) <= len(TIME_ZONES) and int(timeZone) > 0:
                ...
                try:
                    ...
                    if ctx.member.id in memberTimeZones:
                        del memberTimeZones[ctx.member.id]
            with open(MEMBER_TIME_ZONES_FILE, "w") as f:
                json.dump(memberTimeZones, f, indent=4)
