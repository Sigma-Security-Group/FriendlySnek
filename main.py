...
async def on_message(message: discord.Message) -> None:
    ...
    if [True for mention in message.mentions if mention.id == client.user.id]:
        replies = ["snek", "snek!", "snek?", "snek...",
                    ...
                    "Speak to management. Oh wait, that's me.",
...
async def debug(ctx: commands.Context) -> None:
    """Debug command."""
    oldestMessageDate = datetime.datetime(2024, 10, 27, tzinfo=datetime.timezone.utc)

    if ctx.guild is None:
        await ctx.send("Failed to backfill recruitment history: guild not found.")
        return

    channelRecruitmentAndHR = ctx.guild.get_channel(RECRUITMENT_AND_HR)
    if not isinstance(channelRecruitmentAndHR, discord.TextChannel):
        await ctx.send("Failed to backfill recruitment history: Recruitment & HR channel not found.")
        log.exception("debug: channelRecruitmentAndHR not discord.TextChannel")
        return

    await ctx.send("Backfilling recruitment history from Recruitment & HR message history...")

    def getUserIdFromText(text: str | None) -> int | None:
        if not text:
            return None
    ...
    def getUserIdAfterLabel(text: str | None, label: str) -> int | None:
        if not text:
            return None
    ...
    def getIdFromFooter(footerText: str | None, labels: tuple[str, ...]) -> int | None:
        if not footerText:
            return None

        for label in labels:
            match = re.search(rf"{re.escape(label)}\s*:\s*(\d+)", footerText, re.IGNORECASE)
            if match:
                return int(match.group(1))

        return None

    def getRecruitmentHistoryRecord(message: discord.Message) -> dict | None:
        ...
        if "member verified" in title:
            eventType = "verified"
            memberId = getIdFromFooter(footerText, ("Verified ID", "Verified member id"))
            actorId = getIdFromFooter(footerText, ("Interviewer ID",))
            if actorId is None:
                actorId = getUserIdAfterLabel(embed.description, "Interviewer")

        elif "prospect denied" in title:
            eventType = "denied"
            memberId = getIdFromFooter(footerText, ("Prospect ID", "Member id"))
            actorId = getIdFromFooter(footerText, ("Interviewer ID",))
            if actorId is None:
                actorId = getUserIdAfterLabel(embed.description, "Interviewer")

        elif "newcomer onboarded" in title:
            eventType = "newcomer_completed"
            memberId = getIdFromFooter(footerText, ("User ID",))
            for field in embed.fields:
                actorId = getUserIdFromText(field.value)
                if actorId is not None:
                    break
            if actorId is None:
                actorId = getUserIdAfterLabel(embed.description, "Onboarded by")

        if eventType is None or memberId is None or actorId is None:
            return None

        return {
            "eventType": eventType,
            "createdAt": message.created_at.astimezone(datetime.timezone.utc).isoformat(),
            "memberId": memberId,
            "actorId": actorId,
        }

    def getRecruitmentRecordKey(record: dict) -> tuple[str, int, int] | None:
        eventType = record.get("eventType")
        memberId = record.get("memberId")
        actorId = record.get("actorId")
        if not isinstance(eventType, str) or not isinstance(memberId, int) or not isinstance(actorId, int):
            return None

        return (eventType, memberId, actorId)

    def addRecruitmentRecordToIndex(record: dict, index: dict[tuple[str, int, int], list[datetime.datetime]]) -> None:
        key = getRecruitmentRecordKey(record)
        createdAt = record.get("createdAt")
        if key is None or not isinstance(createdAt, str):
            return

        try:
            timestamp = datetime.datetime.fromisoformat(createdAt)
        except ValueError:
            return

        index.setdefault(key, []).append(timestamp)

    def isDuplicateRecruitmentRecord(record: dict, index: dict[tuple[str, int, int], list[datetime.datetime]]) -> bool:
        key = getRecruitmentRecordKey(record)
        if key is None:
            return False

        recordCreatedAt = datetime.datetime.fromisoformat(record["createdAt"])
        return any(abs((recordCreatedAt - existingTimestamp).total_seconds()) <= 10 for existingTimestamp in index.get(key, []))

    try:
        with open(RECRUITMENT_HISTORY_FILE) as f:
            recruitmentHistory = json.load(f)
    except FileNotFoundError:
        recruitmentHistory = []

    if not isinstance(recruitmentHistory, list):
        await ctx.send("Failed to backfill recruitment history: recruitment history file is not a list.")
        log.warning("debug: recruitment history file is not a list")
        return

    recruitmentHistoryIndex: dict[tuple[str, int, int], list[datetime.datetime]] = {}
    for record in recruitmentHistory:
        addRecruitmentRecordToIndex(record, recruitmentHistoryIndex)

    foundRecords = []
    skippedDuplicates = 0
    scannedMessages = 0
    async for message in channelRecruitmentAndHR.history(limit=None):
        if message.created_at < oldestMessageDate:
            break

        scannedMessages += 1
        record = getRecruitmentHistoryRecord(message)
        if record is None:
            continue

        if isDuplicateRecruitmentRecord(record, recruitmentHistoryIndex):
            skippedDuplicates += 1
            continue
    ...
    recruitmentHistory.extend(reversed(foundRecords))
    with open(RECRUITMENT_HISTORY_FILE, "w") as f:
        json.dump(recruitmentHistory, f, indent=4)

    await ctx.send(
        f"Recruitment history backfill complete.\n"
        f"Scanned `{scannedMessages}` messages.\n"
        f"Added `{len(foundRecords)}` records.\n"
        f"Skipped `{skippedDuplicates}` duplicates."
    )
